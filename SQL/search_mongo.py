...
def agg_function(query):
    ...
    partitions = server.mongo_main('getPartitionLocations', filePath)['partitions']
    ...
    for partition in partitions:
        ...
        df= server.mongo_main('readPartition',filePath, None,partition)
...
def select(query):
    ...
    total_json = {}
    filterQuery = utils.buildFilterQuery(columnFilters=columnFilters)
    # child_id = server.getFolderID(filePath)
    # parent_id = server.getFolderID('/'.join(filePath.split('/')[0:-1]))
